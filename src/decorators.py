...
def log(filename=None):
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            start_function_info = f"{func.__name__} started: {time.strftime('%Y-%m-%d %H:%M:%S')} *:{args}, **:{kwargs}"
            ...
            try:
                ...
                time_for_work_info = (f"{func.__name__} finished:{time.strftime('%Y-%m-%d %H:%M:%S')} "
